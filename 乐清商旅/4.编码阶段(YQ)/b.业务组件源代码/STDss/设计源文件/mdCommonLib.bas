...
Public Const cszENdTimeStr = "23:59:59"
...
'将数组转换成字符串,只转换二维数组
Public Function TeamToString2(paszString() As String, Optional pNo As Integer = 1, Optional pbQuotationMarks As Boolean = True) As String
'pNo即第几维
'pbQuotationMarks 是否使用单引号进行分隔
    Dim i As Integer
    Dim sztemp As String
    ...
    Next i
    If nCount > 0 Then
        sztemp = sztemp & szQuotationMarks & Trim(paszString(nCount - 1)) & szQuotationMarks
    End If
    
    TeamToString2 = sztemp
