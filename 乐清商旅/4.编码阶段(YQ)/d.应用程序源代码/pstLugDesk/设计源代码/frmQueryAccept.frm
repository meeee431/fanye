...
Begin VB.Form frmQueryAccept 
   ...
   Begin VB.PictureBox ptQuery 
      ...
      Begin FText.asFlatTextBox txtRoute 
         ...
         OfficeXPColors  =   -1  'True
      End
      Begin FText.asFlatTextBox txtTransportCompany 
         Height          =   315
         Left            =   150
         TabIndex        =   23
         Top             =   780
      ...
      Begin RTComctl3.CoolButton cmdQuery 
         ...
         MCOL            =   12632256
         MPTR            =   1
         MICON           =   "frmQueryAccept.frx":03A6
         PICN            =   "frmQueryAccept.frx":03C2
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
...
Private Sub QueryAcceptLuggage()
    ...
    txtAcceptID.Text = Trim(txtAcceptID.Text)
    ...
    ElseIf optQueryMode7.Value Then
        If Trim(txtTransportCompany.Text) = "" Then
            Set rsTemp = moLugSvr.GetAcceptSheetRS(CDate(dtpStart), CDate(dtpEnd))
        Else
            Set rsTemp = moLugSvr.GetAcceptSheetRS(CDate(dtpStart), CDate(dtpEnd), , , , , , , , , ResolveDisplay(txtTransportCompany.Text))
        End If
        
        avCustmonData(3, 2) = optQueryMode7.Caption & "且参运公司为" & IIf(txtTransportCompany.Text = "", "空", txtTransportCompany.Text)
    ElseIf optQueryMode8.Value Then
        If Trim(txtRoute.Text) = "" Then
            Set rsTemp = moLugSvr.GetAcceptSheetRS(CDate(dtpStart), CDate(dtpEnd))
        Else
            Set rsTemp = moLugSvr.GetAcceptSheetRS(CDate(dtpStart), CDate(dtpEnd), , , , , , , , , , ResolveDisplay(txtRoute.Text))
        End If
        
        avCustmonData(3, 2) = optQueryMode8.Caption & "且线路为" & IIf(txtRoute.Text = "", "空", txtRoute.Text)
    End If
...
Private Sub OptQueryMode1_Click()
    ...
    txtTransportCompany.Visible = False
...
Private Sub optQueryMode7_Click()
    ...
    cboAcceptStatus.Visible = False
    txtAcceptID.Visible = False
    txtTransportCompany.Visible = True
    txtRoute.Visible = False
    lblQueryType.Caption = "参运公司代码(&T):"
End Sub
...
Private Sub txtTransportCompany_ButtonClick()
On Error GoTo ErrHandle
    Dim oShell As New STShell.CommDialog
    Dim aszTemp() As String
    oShell.Init m_oAUser
    ...
    Set oShell = Nothing
    If ArrayLength(aszTemp) = 0 Then Exit Sub
    txtTransportCompany.Text = aszTemp(1, 1) & "[" & Trim(aszTemp(1, 2)) & "]"
    cmdQuery.Enabled = True
Exit Sub
ErrHandle:
ShowErrorMsg
