...
Begin VB.Form frmUpdateSheet 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin MSComCtl2.DTPicker dtpUpLoadDate 
         ...
         CurrentDate     =   39113
      End
      Begin FCmbo.asFlatCombo cboEndStation 
         Height          =   270
         Left            =   4470
         TabIndex        =   53
         Top             =   150
...
Private Sub cboEndStation_Click()
    If cboEndStation.Text <> "[保持原站]" Then ShowMsg "更改到达站后，该签发单里所含的所有受理单里的到达站将统一改为所选的到达站，请谨慎操作！"
End Sub
...
' If KeyCode = 13 Then
  ...
  If rsTemp.RecordCount = 0 Then
     ...
     Exit Sub
  End If
  If Trim(FormatDbValue(rsTemp!up_load_time)) <> "" Then
     MsgBox "此签发单已经过确认！", vbInformation, Me.Caption
  End If
''     cmdOK.Enabled = False
...
'     lblVehicle.Caption = Trim(rsTemp!license_tag_no)
...
'  lblSheetID.Caption = Trim(rsTemp!sheet_id)
  ...
  txtVehicle.Text = MakeDisplayString(FormatDbValue(rsTemp!vehicle_id), FormatDbValue(rsTemp!license_tag_no))
  
  '列出途径站
    
    oLugSvr.Init m_oAUser
    aszTemp = oLugSvr.GetBusStationNamesEx(Format(rsTemp!bus_date, "yyyy-MM-dd"), txtBusID.Text)
    cboEndStation.Clear
    cboEndStation.AddItem "[保持原站]"
    For i = 1 To ArrayLength(aszTemp)
        cboEndStation.AddItem aszTemp(i)
    Next i
    cboEndStation.ListIndex = 0
    
    '装车时间
    If FormatDbValue(rsTemp!up_load_time) = "" Then
        dtBusStartTime = GetAllotStationBusStartTime(txtBusID.Text, Format(rsTemp!bus_date, "yyyy-MM-dd"))
        If dtBusStartTime <> cdtEmptyDate Then
            dtpUpLoadDate.Value = dtBusStartTime
'        Else
'            ShowMsg "无效车次或车次已停班！"
        End If
    Else
        dtpUpLoadDate.Value = FormatDbValue(rsTemp!up_load_time)
    End If
...
On Error GoTo here
    ...
    If lblSheetID.Caption <> cboSheetID.Text Then
       MsgBox "交接清单上的信息与选中的单号不符，可能是由于选择单号后没有按回车键进行刷新！", vbExclamation, Me.Caption
       Exit Sub
    End If
    
    mAnswer = MsgBox("您输入的车号是  " & Trim(txtVehicle.Text) & "   是否确认？", vbInformation + vbYesNo, Me.Caption)
    If mAnswer = vbYes Then
'    moCarrySheet.SheetID = Trim(cboSheetID.Text)
    moLugSvr.UpdateCarryLuggage Trim(cboSheetID.Text), ResolveDisplay(txtVehicle.Text), ResolveDisplayEx(txtVehicle.Text), IIf(cboEndStation.Text = "[保持原站]", "", cboEndStation.Text), dtpUpLoadDate.Value
...
Private Sub Form_Load()
  ...
  Dim i As Integer
     ...
     AlignFormPos Me
     Top = frmCarryLuggage.fraOutLine.Top + 1700
     Left = frmCarryLuggage.fraOutLine.Left
     ...
     cboEndStation.AddItem "[保持原站]"
     cboEndStation.ListIndex = 0
     frmCarryLuggage.FillAllVehicle
End Sub
...
On Error GoTo ErrHandle
    ...
    aszTemp = oShell.SelectREBus(CDate(lblDate.Caption), True, False)
    Set oShell = Nothing
    ...
    Dim oLugSvr As New LuggageSvr
    oLugSvr.Init m_oAUser
    aszTemp = oLugSvr.GetBusStationNamesEx(Format(dtpUpLoadDate.Value, "yyyy-MM-dd"), txtBusID.Text)
    cboEndStation.Clear
    cboEndStation.AddItem "[保持原站]"
    For i = 1 To ArrayLength(aszTemp)
        cboEndStation.AddItem aszTemp(i)
    ...
    '装车时间
    dtBusStartTime = GetAllotStationBusStartTime(txtBusID.Text, Format(dtpUpLoadDate.Value, "yyyy-MM-dd"))
    If dtBusStartTime <> cdtEmptyDate Then
        dtpUpLoadDate.Value = dtBusStartTime
    Else
'        ShowMsg "无效车次或车次已停班！"
...
'
'    oLugSvr.Init m_oAUser
'    aszTemp = oLugSvr.GetBusStationNamesEx(Format(Now, "yyyy-MM-dd"), txtBusID.Text)
'    cboEndStation.Clear
'    cboEndStation.AddItem "[保持原站]"
'    For i = 1 To ArrayLength(aszTemp)
'        cboEndStation.AddItem aszTemp(i)
