...
'
'    oREBus.Init m_oUser
'    oREBus.Identify m_szBusID, m_dBusDate, g_tCheckInfo.CheckGateNo
'
...
On Error GoTo ErrorHandle
    ...
    Dim oVehicle As New Vehicle
    Dim oRoute As New Route
    Dim atAllotInfo() As TReBusAllotInfo
    ...
    lblRoute.Caption = oRoute.RouteName
    lblTotalSeat.Caption = oREBus.TotalSeat
    If m_oUser.SellStationID = "" Then
        lblSaleSeat.Caption = oREBus.SaledSeatCount
        lblCheckGate.Caption = oREBus.CheckGate
        lblStartTime.Caption = Format(oREBus.StartUpTime, "hh:mm:ss")
    Else
        atAllotInfo = oREBus.GetAllotInfo(m_oUser.SellStationID)
        nCount = ArrayLength(atAllotInfo)
        If nCount = 0 Then
            MsgBox "该车次设置的配载信息不正确", vbInformation, Me.Caption
            Exit Sub
        End If
        For i = 1 To nCount
            If m_oUser.SellStationID = atAllotInfo(i).szSellStationID Then
                lblSaleSeat.Caption = atAllotInfo(i).nSellQuantity
                lblCheckGate.Caption = atAllotInfo(i).szCheckGateName
                lblStartTime.Caption = Format(atAllotInfo(i).dtRunTime, "hh:mm:ss")
                Exit For
            End If
        Next i
    End If
