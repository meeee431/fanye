...
Begin VB.Form frmRePrintSheet 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin RTComctl3.CoolButton lblBusCheckInfo 
         ...
         MCOL            =   12632256
         MPTR            =   1
         MICON           =   "frmRePrintSheet.frx":0A02
         PICN            =   "frmRePrintSheet.frx":0A1E
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Image Image1 
         Height          =   480
         Left            =   225
         Picture         =   "frmRePrintSheet.frx":0B78
         Top             =   285
         Width           =   480
      End
      Begin VB.Label Label2 
         BackStyle       =   0  'Transparent
         Caption         =   "  重打路单将生成新的路单编号，以便与打印机的当前路单编号一致，请在路单打印错误时才使用此功能，正常时请勿使用。"
         Height          =   555
         Left            =   960
         TabIndex        =   7
         Top             =   270
   ...
   Begin RTComctl3.CoolButton cmdPreView 
      ...
      MCOL            =   12632256
      MPTR            =   1
      MICON           =   "frmRePrintSheet.frx":1442
      UMCOL           =   -1  'True
      SOFT            =   0   'False
      PICPOS          =   0
      NGREY           =   0   'False
   ...
   Begin RTComctl3.CoolButton cmdExit 
      ...
      MCOL            =   12632256
      MPTR            =   1
      MICON           =   "frmRePrintSheet.frx":145E
      UMCOL           =   -1  'True
      SOFT            =   0   'False
      PICPOS          =   0
      NGREY           =   0   'False
   ...
   Begin RTComctl3.CoolButton cmdRePrintSheet 
      ...
      MCOL            =   12632256
      MPTR            =   1
      MICON           =   "frmRePrintSheet.frx":147A
      UMCOL           =   -1  'True
      SOFT            =   0   'False
      PICPOS          =   0
      NGREY           =   0   'False
...
Private Sub cmdRePrintSheet_Click()
    ...
    If ToDBDate(lblDate.Caption) <> ToDBDate(oSystemParam.NowDate) Then
        MsgboxEx "不能重打非当日车次的路单！", vbExclamation + vbOKOnly
        Exit Sub
    End If
    ...
    End If
    If txtOriSheetNo.Text = "" Then
        MsgboxEx "请输入需重打的路单号", vbInformation, "重打路单"
        txtOriSheetNo.SetFocus
        Exit Sub
    End If
    If lblCurSheetNo.Caption <> txtOriSheetNo.Text Then
        If MsgboxEx("* 重打路单将作废原路单!" & vbCrLf & "* 请确定重打的路单编号同打印机上的路单号一致，继续吗?", vbExclamation + vbYesNo, msgTitle) = vbYes Then
        
            If Trim(mtOriSheetInfo.szSettleSheetID) <> "" Then
                If MsgboxEx("路单已经结算，要继续作废吗？", vbExclamation + vbYesNo + vbDefaultButton2, msgTitle) = vbNo Then Exit Sub
            End If
            g_oChkTicket.ChangeCheckSheetNo txtOriSheetNo.Text, lblCurSheetNo.Caption
            ...
            ofrmTmp.mbNoPrintPrompt = True
            ofrmTmp.mbExitAfterPrint = False
            ofrmTmp.mszSheetID = lblCurSheetNo.Caption
            ofrmTmp.Show vbModal
