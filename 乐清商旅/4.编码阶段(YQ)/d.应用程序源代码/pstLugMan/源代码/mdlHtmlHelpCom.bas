Attribute VB_Name = "mdlHtmlHelpCom"
...
'
'Public Sub HTMLShowTopic(ByVal intHelpFile As Integer, _
'    strWindow As String, strTopic As String)
'
'  ' Force the Help window to load a specific topic.
'  ' The Help window will synchronize the
'  ' Contents display automatically
'  htmlHelpTopic hwnd, SetHTMLHelpStrings(intHelpFile) _
'       & ">" & strWindow, HH_DISPLAY_TOPIC, strTopic
'
'End Sub
...
Public HtmlHelpName As String
Public HtmlHelpWinType As String
Public TopicID As Long

Public Const clSupportID = 1101
'For GetWindowsDir
Private lForWindir As Long
Private Declare Function GetWindowsDirectory Lib "kernel32" Alias "GetWindowsDirectoryA" (ByVal lpBuffer As String, ByVal nSize As Long) As Long
Const MAX_PATH = 260


Public Function SetHTMLHelpStrings(Optional ByVal HelpFile As String = "STHelp.chm", Optional HelpWinType As String = "STHelp") As String
    SetHTMLHelpStrings = GetWinDir & "\Help\STHelp\" & HelpFile
'    SetHTMLHelpStrings = "D:\客运综合管理平台2.0\8.产品文档\帮助系统\综合调度\设计源文件\" & HelpFile
    HtmlHelpName = HelpFile
    HtmlHelpWinType = HelpWinType
    On Error Resume Next
    HtmlHelpWinType = Left(HelpFile, (Len(HelpFile) - 4))
    
End Function


Public Sub CallHTMLHelpIndex()
    Call HTMLHelpIndex(HtmlHelpName, HtmlHelpWinType)
End Sub

Public Sub CallHTMLShowTopicID()
    Call HTMLShowTopicID(HtmlHelpName, HtmlHelpWinType, TopicID)
End Sub


Private Sub HTMLHelpIndex(ByVal HelpName As String, WinType As String)
    On Error GoTo here
    htmlhelp hWnd, SetHTMLHelpStrings(HelpName) & ">" & WinType, HH_DISPLAY_INDEX, 0
    Exit Sub
here:
    MsgBox "帮助文件未找到,拷贝对应的帮助文件到<Windows目录>的Help\SNHelp下,如:" & vbCrLf & "C:\Windows\Help\SNHelp\SNSysMan.chm", vbExclamation, "帮助文件未找到！"
End Sub

Private Sub HTMLShowTopicID(ByVal HelpName As String, WinType As String, lngTopicID As Long)
    On Error GoTo here
    htmlhelp hWnd, SetHTMLHelpStrings(HelpName) & ">" & WinType, HH_HELP_CONTEXT, lngTopicID
Exit Sub
here:
    MsgBox "帮助文件未找到,拷贝对应的帮助文件到<Windows目录>的Help\SNHelp下,如:" & vbCrLf & "C:\Windows\Help\SNHelp\SNSysMan.chm", vbExclamation, "帮助文件未找到！"
End Sub
...
Public Function GetWinDir() As String
    ...
    lCount = GetWindowsDirectory(szTemp, MAX_PATH)
    If lCount <> 0 Then
        GetWinDir = Left(szTemp, lCount)
    Else
        GetWinDir = ""
    End If
End Function
'显示HTMLHELP,直接拷贝
Public Sub DisplayHelp(frmTemp As Form, Optional HelpType As EHelpType = content)
