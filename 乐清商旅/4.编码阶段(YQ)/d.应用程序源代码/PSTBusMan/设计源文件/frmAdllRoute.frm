...
Private Sub FillItem(paszRoute() As String, Optional pbIsUpdate As Boolean = False)
    ...
    For i = 1 To nCount
        ...
        If Val(paszRoute(i, 5)) <> 0 Then
            liTemp.SubItems(6) = "正常"
            liTemp.SubItems(7) = paszRoute(i, 3)
        Else
            liTemp.SubItems(6) = "维修"
            liTemp.SubItems(7) = paszRoute(i, 3)
            liTemp.SmallIcon = "Stop"
            SetListViewLineColor lvRoute, liTemp.Index, vbRed
        End If
        If paszRoute(i, 9) = True Then
            liTemp.SubItems(5) = "是"
            liTemp.ListSubItems.Item(5).ForeColor = vbRed
        Else
            liTemp.SubItems(5) = "否"
            liTemp.ListSubItems.Item(5).ForeColor = vbBlack
        End If
    Next
    If nCount > 1 Then
...
'    Set MDIScheme.ceReport.ListViewSource = lvRoute
