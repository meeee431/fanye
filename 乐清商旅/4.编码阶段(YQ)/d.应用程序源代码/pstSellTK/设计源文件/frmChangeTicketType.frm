...
On Error GoTo here
    ...
    'Set oCTicket = m_oSell.GetTicketClient(txtTicketID.Text)  zyw玉环邮政用不到，这句有反而会影响邮政售出的票，车站不能改
        ...
        Else
            ...
            If (oTicket.TicketStatus And ST_TicketNormal) <> 0 Then
                szTemp = "正常售出"
            Else
                szTemp = "改签售出"
            End If
    
            If (oTicket.TicketStatus And ST_TicketCanceled) <> 0 Then
                szTemp = szTemp & "/已废"
            ElseIf (oTicket.TicketStatus And ST_TicketChanged) <> 0 Then
                szTemp = szTemp & "/已被改签"
            ElseIf (oTicket.TicketStatus And ST_TicketChecked) <> 0 Then
                szTemp = szTemp & "/已检"
            ElseIf (oTicket.TicketStatus And ST_TicketReturned) <> 0 Then
                szTemp = szTemp & "/已退"
            End If
