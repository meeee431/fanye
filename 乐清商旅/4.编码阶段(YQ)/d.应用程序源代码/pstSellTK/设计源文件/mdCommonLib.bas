...
Public Function NumArray(paszIn As Variant) As Integer '判断指定数组中不同内容的个数
    Dim i As Integer
    Dim j As Integer
    j = 1
    If ArrayLength(paszIn) <= 1 Then Exit Function
    For i = 1 To ArrayLength(paszIn) - 1
        If paszIn(i) <> paszIn(i + 1) And paszIn(i + 1) <> "" Then
            j = j + 1
        End If
    Next
    NumArray = j
End Function
Public Function BArray(paszIn As Variant, sztemp As String) As Boolean '判断指定数组中所传入的字符串是否已存在
    Dim i As Integer
    Dim n As Integer
    If ArrayLength(paszIn) = 1 Then
            n = 0
    Else
            n = 1
    End If
    For i = 1 To ArrayLength(paszIn) - n
        If paszIn(i) <> sztemp Then
            BArray = False
        Else
            BArray = True
            Exit Function
