...
Begin VB.Form frmVehicleBalance 
   BackColor       =   &H00E0E0E0&
   ...
   ClientTop       =   3930
   ClientWidth     =   5790
   Icon            =   "frmVehicleBalance.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.PictureBox ptTitle 
      ...
      Begin VB.Image Image1 
         Height          =   855
         Left            =   1380
         Picture         =   "frmVehicleBalance.frx":000C
         Top             =   0
         Width           =   5925
      End
   ...
   Begin RTComctl3.CoolButton cmdCancel 
      ...
      MCOL            =   12632256
      MPTR            =   1
      MICON           =   "frmVehicleBalance.frx":14F6
      UMCOL           =   -1  'True
      SOFT            =   0   'False
      PICPOS          =   0
      NGREY           =   0   'False
   ...
   Begin RTComctl3.CoolButton cmdOk 
      ...
      MCOL            =   12632256
      MPTR            =   1
      MICON           =   "frmVehicleBalance.frx":1512
      UMCOL           =   -1  'True
      SOFT            =   0   'False
      PICPOS          =   0
      NGREY           =   0   'False
   ...
   End
End
Attribute VB_Name = "frmVehicleBalance"
...
Private Sub cmdok_Click()
    ...
    Set rsSale = oDss.GetVehicleStatByBusDate("", dtpStartDate.Value, dtpEndDate.Value, TP_AllSold)
    Set rsCheck = oReport.GetCheckVehicleStatByBusDate("", dtpStartDate.Value, dtpEndDate.Value)
    
    
    '此处查询结算,可能与要求不一致,应该查询的是所有已结算的,还是===
    Set rsSettle = oReport.VehicleSettleStat(dtpStartDate.Value, DateAdd("d", 1, dtpEndDate.Value), "", "", CS_QueryAll, CS_SettleSheetNotInvalid, False)
...
'    Dim lSaleQuantity As Long
'    Dim dbSaleTicketPrice As Double
'    Dim lReturnNum As Long
'    Dim dbReturnCharge As Double
'    Dim lSaleTotalQuantity As Long
'    Dim dbSaleTotalTicketPrice As Double
'
...
'
'    Dim lSettleQuantity As Long
''    Dim dbSettleTicketPrice As Double
'    Dim dbSplitItem(20) As Double
'    Dim lLeftQuantity As Long
'    Dim dbLeftTicketPrice As Double
'

    With rsTemp.Fields
        .Append "vehicle_id", adChar, 5 '车辆代码
        .Append "license_tag_no", adChar, 10 '车牌号
...
'    rsTemp.Open
    ...
    prsCheck.MoveFirst
    '合并售票及检票
    Set prsSale = MergeMultiRecord(prsSale, prsCheck, "vehicle_id")
    '再与结算进行合并
    Set prsSale = MergeMultiRecord(prsSale, prsSettle, "vehicle_id")
    ...
    For i = 1 To prsSale.RecordCount
        rsTemp.AddNew
        rsTemp!vehicle_id = prsSale!vehicle_id
        rsTemp!license_tag_no = prsSale!license_tag_no
...
Private Function MergeSale(prsData As Recordset) As Recordset
    ...
    For i = 1 To prsData.RecordCount
        ...
        rsTemp.AddNew
        rsTemp!vehicle_id = FormatDbValue(prsData!vehicle_id)
        rsTemp!license_tag_no = FormatDbValue(prsData!license_tag_no)
...
Private Function MergeCheck(prsData As Recordset) As Recordset
    ...
    For i = 1 To prsData.RecordCount
        If szVehicleID <> FormatDbValue(prsData!vehicle_id) Then
            '赋予记录集
