...
'解析字符串位字符数组xxd,
Public Function StringToTeamRX(pszString As String, pszLen As String) As String()
    'pszstirng传入需解析的字符串
    'pszlen需以几个字符并为一个数组
    Dim i As Integer
    Dim aszTemp() As String
    Dim nLen As Integer
    Dim j As Integer
    nLen = Len(Trim(pszString)) / pszLen
    ReDim aszTemp(1 To nLen)
    j = 1
    For i = 1 To nLen
        aszTemp(i) = Mid(pszString, j, pszLen)
        j = j + pszLen
    Next

    StringToTeamRX = aszTemp
End Function
