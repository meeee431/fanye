...
Public Const INTERNETSELL = "2001" '网上售票
Public Const INTERNETCANCEL = "2002" '网上退票
Public Const AUTOCANCELINTTK = "8888" '网上自动退票
Public Const AUTOCANCELTKID = "9999" '自动废票
Public Const GetInterNetTK = "2003" '取网上售票
'=============================================================
'其他常量定义
'=============================================================
...
Private Sub SetSellTicketValue(pszTicketID As String, pnTicketType As Integer, pszStartStationID As String, pszEndStationID As String, pszBusID As String _
    , pnTicketNum As Integer, pdyBusDate As Date, pszSeatID As String)
...
Public Function GetSellTicketRequestStr(pszTicketID As String, pnTicketType As Integer, pszStartStationID As String, pszEndStationID As String, pszBusID As String _
    , pnTicketNum As Integer, pdyBusDate As Date, pszSeatID As String) As String
    
    SetSellTicketValue pszTicketID, pnTicketType, pszStartStationID, pszEndStationID, pszBusID, pnTicketNum, pdyBusDate, pszSeatID
...
Public Function GetBusOffTime(ByVal pszStr As String) As String
    '车次发车时间HHMM
    GetBusOffTime = Trim(MidA(pszStr, cnPosBusOffTime, cnLenBusOffTime + 1))
    GetBusOffTime = Left(GetBusOffTime, 2) & ":" & Right(GetBusOffTime, 2)
End Function

Public Function GetSeatID(ByVal pszStr As String) As String
    '座位
    GetSeatID = Trim(MidA(pszStr, cnPosSeatID + 1, cnLenSeatID))
End Function
...
Public Function GetReserved(pszStr As String) As String
    '预留
    GetReserved = Mid(pszStr, cnPosReserved, cnLenReserved)
End Function
...
Public Function ConvertCheckSTRToString(prsCheckGateRS() As String) As String
    Dim szStr As String
    Dim i As Integer
    szStr = ""

'        .MoveFirst
        For i = 1 To ArrayLength(prsCheckGateRS)
            szStr = szStr & FormatLen(prsCheckGateRS(i, 1), cnStationLenID)
            szStr = szStr & FormatLen(prsCheckGateRS(i, 2), cnStationLenName)
            szStr = szStr & "|"
        Next i

    ConvertCheckSTRToString = szStr
End Function
...
'将车次查询调用返回的记录集转换成字符串
Public Function ConvertBusRSToString(ByVal prsBusRS As ADODB.Recordset, ByRef nCount As Integer, szSellStationID As String) As String
...
'        .MoveFirst
        For i = 1 To .RecordCount
            If FormatDbValue(!Status) = 0 And FormatDbValue(!sell_station_id) = szSellStationID Then
    ...
    '        szStr = szStr & FormatLen(TimeToPackage(!bus_start_time), cnBusRsLen_bus_start_time)
            szStr = szStr & FormatLen(!bus_type, cnBusRsLen_bus_kind)
...
'    szTemp = Right(szTemp, LenA(szTemp) - nLen)
    ...
    With rsTemp
        ...
        .Fields.Append "bus_kind", adInteger
    ...
    With rsTemp
        ...
        For i = 1 To ArrayLength(aszBus) - 1 - 1 '因为最后有一个"|",故还需要减1
            ...
            .Fields("bus_kind") = MidA(aszBus(i), cnBusRsPos_bus_kind, cnBusRsLen_bus_kind)
...
'将明细查询调用返回的记录集转换成字符串
Public Function ConvertSellDetailRSToString(ByVal prsSellDetailRS As ADODB.Recordset) As String

    Dim szStr As String
    Dim i As Integer
    szStr = ""
    With prsSellDetailRS
'        .MoveFirst
        For i = 1 To .RecordCount
            szStr = szStr & FormatLen(!id, 10)
            szStr = szStr & FormatLen(Format(!StartTime, "yyyymmddhhmm"), 12)
            szStr = szStr & FormatLen(!scheduleid, 10)
            szStr = szStr & FormatLen(MidA(!startname, 1, 20), 20)
            szStr = szStr & FormatLen(MidA(!destname, 1, 10), 10)
            szStr = szStr & FormatLen(!TicketType, 1)
            szStr = szStr & FormatLen(!OperatorId, 10)
            szStr = szStr & FormatLen(!seatid, 2)
            szStr = szStr & FormatLen(MoneyToPackage(!Price), 10)
            szStr = szStr & FormatLen(MidA(!Status, 1, 4), 4)
            szStr = szStr & FormatLen(Format(!selldate, "yyyymmddhhmm"), 12)
            szStr = szStr & "|"
            .MoveNext
        Next i
    End With
    ConvertSellDetailRSToString = szStr
End Function


'将统计查询调用返回的记录集转换成字符串
Public Function ConvertSellCountRSToString(ByVal prsSellDetailRS As ADODB.Recordset) As String
    ...
    With prsSellDetailRS
        .MoveFirst
        For i = 1 To .RecordCount
            szStr = szStr & FormatLen(!OperatorId, 20)
            szStr = szStr & FormatLen(!ticket_num, 10)
            szStr = szStr & FormatLen(!total_price, 20)
            szStr = szStr & FormatLen(!cancel_ticket_num, 10)
            szStr = szStr & FormatLen(!cancel_total_price, 20)
            szStr = szStr & FormatLen(!min_ticket_id, 10)
            szStr = szStr & FormatLen(!max_ticket_id, 10)
            szStr = szStr & "|"
            .MoveNext
        Next i
    End With
    ConvertSellCountRSToString = szStr
End Function

'将售票明细查询调用返回的字符串转换成记录集
Public Function ConvertStringToSellDetailRS(ByVal pszStr As String) As ADODB.Recordset
    '与ConvertStationRSToString是逆向操作
    Dim szTemp As String
    Dim nLen As Integer '存放传输前缀的长度
    Dim aszStation() As String
...
'    szTemp = Right(szTemp, LenA(szTemp) - nLen)
    ...
    '创建记录集
    With rsTemp
        .Fields.Append "id", adChar, 10
        .Fields.Append "StartTime", adChar, 12
        .Fields.Append "scheduleid", adChar, 10
    
        .Fields.Append "startname", adChar, 20
        .Fields.Append "destname", adChar, 10
        .Fields.Append "TicketType", adChar, 1
        
        .Fields.Append "operatorid", adChar, 10
        .Fields.Append "seatid", adChar, 2
        .Fields.Append "price", adChar, 10
        
        .Fields.Append "Status", adChar, 4
        .Fields.Append "selldate", adChar, 12
    ...
    With rsTemp
        ...
        For i = 1 To ArrayLength(aszStation) - 1 - 1 '从1 开始是因为第一个"|"前面有垃圾数据      因为最后有一个"|",故还需要减1
            .AddNew
            .Fields("id") = MidA(aszStation(i), 1, 10)
            .Fields("StartTime") = MidA(aszStation(i), 11, 12)
            .Fields("scheduleid") = MidA(aszStation(i), 23, 10)
            
            .Fields("startname") = MidA(aszStation(i), 33, 20)
            .Fields("destname") = MidA(aszStation(i), 53, 10)
            .Fields("TicketType") = MidA(aszStation(i), 63, 1)
            
            .Fields("operatorid") = MidA(aszStation(i), 64, 10)
            .Fields("seatid") = MidA(aszStation(i), 74, 2)
            .Fields("price") = MidA(aszStation(i), 76, 10)
            
            .Fields("Status") = MidA(aszStation(i), 86, 4)
            .Fields("selldate") = MidA(aszStation(i), 90, 12)
            
            .Update
        Next i
    End With
    Set ConvertStringToSellDetailRS = rsTemp
