...
Public Const cszRegKeyCompany = "Software"       '公司注册键
Public Const cszRegKeyProduct = "StationNet"             '产品注册键
...
On Error GoTo ErrHandle
    ...
    If dbValue <> 0 Or Left(pszText, 3) = "0.0" Or pszText = "0" Or pszText = "0." Then
        
        GetTextToNumeric = Trim(str(dbValue))
'        If Abs(dbValue) < 1 And Abs(dbValue) > 0 Then GetTextToNumeric = "0" & GetTextToNumeric
        If Right(pszText, 1) = "." And pbCanBeDecimal And dbValue = Int(dbValue) Then
            GetTextToNumeric = GetTextToNumeric & "."
        Else
            '后尾加载0
            If dbValue = Int(dbValue) And Len(Trim(dbValue)) <> Len(pszText) Then
...
Public Function ApartBaseFig(ByVal nNumber As String, Optional ByVal bolRead As Boolean = False) As String() 'nNumber<9999
    ...
    For i = nLength To 1 Step -1
        j = j + 1
        If IsNumeric((Mid(szTemp, i, 1))) = True Then
            nBit = CInt(Mid(szTemp, i, 1))
            szResult(j) = szNum(nBit)
        ElseIf Mid(szTemp, i, 1) = szPoint Then
            nBit = 10
            j = 0
            szResult(1) = szNum(10)
        ...
        If nBit = 0 Then
            ...
            bolZero = True
        ElseIf nBit = 10 Then
            szReadResult = szResult(1) & szBit(1) & szReadResult
            bolLastZero = False
            bolZero = False
        Else
...
'将数组转换成字符串,只转换二维数组
Public Function TeamToString(paszString() As String, Optional pNo As Integer = 1, Optional pbQuotationMarks As Boolean = True) As String
'pNo即第几维
'pbQuotationMarks 是否使用单引号进行分隔
    Dim i As Integer
    Dim szTemp As String
    Dim nCount As Integer
    Dim szQuotationMarks As String
    nCount = ArrayLength(paszString)
    
    If pbQuotationMarks Then
        szQuotationMarks = "'"
    Else
        szQuotationMarks = ""
    End If
    For i = 1 To nCount - 1
        szTemp = szTemp & szQuotationMarks & Trim(paszString(i, pNo)) & szQuotationMarks & ","
    Next i
    If nCount > 0 Then
        szTemp = szTemp & szQuotationMarks & Trim(paszString(i, pNo)) & szQuotationMarks
    End If
...
'将数组转换成字符串,只转换一维数组
Public Function TeamToString_OneDimension(paszString() As String, Optional pbQuotationMarks As Boolean = True) As String
'pNo即第几维
'pbQuotationMarks 是否使用单引号进行分隔
    Dim i As Integer
    Dim szTemp As String
    ...
    End If
    For i = 1 To nCount - 1
        szTemp = szTemp & szQuotationMarks & Trim(paszString(i)) & szQuotationMarks & ","
    Next i
    If nCount > 0 Then
        szTemp = szTemp & szQuotationMarks & Trim(paszString(i)) & szQuotationMarks
    End If
    
    TeamToString_OneDimension = szTemp
...
'          vTemp = CInt(Val(szPrefix)) * 60 + CInt(Val((Left(szSuffix, 1)))) * 10 + CInt(Val((Right(szSuffix, 1))))
'        Else
'          vTemp = CInt(Val(szPrefix)) * 60 + CInt(Val(szSuffix))
'        End If
'    Else
...
'    GetStopTime = vTemp
    ...
    If pbIsRegular Then
        If Left(szPrefix, 1) = "0" Then
            vTemp = CInt(Val(szPrefix)) * 60 + CInt(Val((Left(szSuffix, 1)))) / 10 * 60 + CInt(Val((Right(szSuffix, 1))))
        Else
            vTemp = CInt(Val(szPrefix)) * 60 + CInt(Val((Left(szSuffix, 1)))) / 10 * 60 + CInt(Val((Right(szSuffix, 1))))
        End If
    Else
...
''''转换字符串
''将01,02这样的字符串转换为'01','02'
Public Function TransformString(pszSource As String) As String

    pszSource = Replace(pszSource, "'", "")
    
    If Left(pszSource, 1) = "," Then pszSource = Right(pszSource, Len(pszSource) - 1)
    If Right(pszSource, 1) = "," Then pszSource = Left(pszSource, Len(pszSource) - 1)
    ...
    If Left(pszSource, 1) <> "'" Then pszSource = "'" & pszSource
    
    If Right(pszSource, 1) <> "'" Then pszSource = pszSource & "'"
    
    TransformString = pszSource
    
End Function

'''''判断字符串
''判断01是否在01,02这样的字符串中
Public Function CheckString(ByVal pszSource As String, ByVal pszStr As String) As Boolean

    pszSource = TransformString(pszSource)
    pszStr = TransformString(pszStr)
    
    If InStr(pszSource, pszStr) <> 0 Then
        CheckString = True
    Else
        CheckString = False
    End If
...
Public Function MergeMultiRecord(prsDataSet1 As Recordset, prsDataSet2 As Recordset, pszMergeKeyField As String, Optional pbDescSort As Boolean, Optional pbLikePrev As Boolean) As Recordset
'如果需要按多个字段进行合并,则pszMergeKeyField传入时按"bus_id,transport_company_id"这种方式进行传入.

'两个记录集必须都按pszMergeKeyField进序
    'pbDescSort是否降序排列
    'pbLikePrev当两个记录集存在同一键值一对多的情况时，复杂一的记录行，使其数量与多的行匹配，否则后续置空
    

On Error Resume Next

    '合并成新记录集
    Dim rsMerged As New Recordset
    Dim i As Integer
    Dim oField As Field
    Dim aszMergeKeyField() As String  '把参数用逗号分隔开的字符串转为数组
    Dim j As Integer
    Dim bFind As Boolean
    Dim nCount As Integer
    
    
    aszMergeKeyField = StringToTeam(pszMergeKeyField) '把参数用逗号分隔开的字符串转为数组
    nCount = ArrayLength(aszMergeKeyField)
    
    For Each oField In prsDataSet1.Fields
    '为了处理Numeric字段不能赋值的BUG
        If oField.Type = adNumeric Then
            rsMerged.Fields.Append oField.Name, adDouble
        Else
            rsMerged.Fields.Append oField.Name, oField.Type, oField.DefinedSize
        End If
    Next
    For Each oField In prsDataSet2.Fields
        If oField.Type = adNumeric Then
            rsMerged.Fields.Append oField.Name, adDouble
        Else
            rsMerged.Fields.Append oField.Name, oField.Type, oField.DefinedSize
    ...
    '处理记录未定义的情况
    If prsDataSet1 Is Nothing Then
        
        Set MergeMultiRecord = prsDataSet2
        Exit Function
    End If
    If prsDataSet2 Is Nothing Then
        Set MergeMultiRecord = prsDataSet1
        Exit Function
    End If
    '处理记录无字段的情况
    If prsDataSet1.Fields.Count = 0 Then Set MergeMultiRecord = prsDataSet2: Exit Function
    If prsDataSet2.Fields.Count = 0 Then Set MergeMultiRecord = prsDataSet1: Exit Function
    
    
    Dim vLastKeyValue1(), vLastKeyValue2()      '前一主键值
    ReDim vLastKeyValue1(1 To nCount)
    ReDim vLastKeyValue2(1 To nCount)
    For j = 1 To nCount
        vLastKeyValue1(j) = Null
        vLastKeyValue2(j) = Null
    Next j
    If prsDataSet1.RecordCount > 0 Then prsDataSet1.MoveFirst
    If prsDataSet2.RecordCount > 0 Then prsDataSet2.MoveFirst
    Do While Not (prsDataSet1.EOF And prsDataSet2.EOF)
        '当一个记录EOF时，另一个记录全部赋给合并集
        If prsDataSet1.EOF Then
            While Not prsDataSet2.EOF
                rsMerged.AddNew
                bFind = True
                For j = 1 To nCount
                    If Not IsNull(vLastKeyValue1(j)) Then
                    Else
                        bFind = False
                        Exit For
                    End If
                ...
                If bFind Then
                    ...
                    For j = 1 To nCount
                        If vLastKeyValue1(j) = prsDataSet2(aszMergeKeyField(j)) Then
                        Else
                            bFind = False
                            Exit For
                        End If
                    ...
                    If bFind Then
                        ...
                        prsDataSet1.MovePrevious
                        If pbLikePrev Then
                            For Each oField In prsDataSet1.Fields
                                rsMerged.Fields(oField.Name) = oField.Value
                            Next
                        End If
                        prsDataSet1.MoveNext
                    End If
                End If
                ...
                Next
                rsMerged.Update
                prsDataSet2.MoveNext
            Wend
            Exit Do
        End If
        If prsDataSet2.EOF Then
            While Not prsDataSet1.EOF
                rsMerged.AddNew
                bFind = True
                For j = 1 To nCount
                    If Not IsNull(vLastKeyValue2(j)) Then
                    Else
                        bFind = False
                        Exit For
                    End If
                ...
                If bFind Then
                    ...
                    For j = 1 To nCount
                        If vLastKeyValue2(j) = prsDataSet1(aszMergeKeyField(j)) Then
                        Else
                            bFind = False
                            Exit For
                        End If
        ...
        For j = 1 To nCount
            If FormatDbValue(prsDataSet1.Fields(aszMergeKeyField(j))) = FormatDbValue(prsDataSet2.Fields(aszMergeKeyField(j))) Then
            Else
                bFind = False
                Exit For
            End If
        ...
        If bFind Then
            ...
            rsMerged.Update
            For j = 1 To nCount
                vLastKeyValue1(j) = prsDataSet1.Fields(aszMergeKeyField(j))
                vLastKeyValue2(j) = prsDataSet2.Fields(aszMergeKeyField(j))
            Next j
            prsDataSet1.MoveNext
            prsDataSet2.MoveNext
            GoTo NextLoop
        ...
        For j = 1 To nCount
            '如果它和上一条一样
            If prsDataSet1.Fields(aszMergeKeyField(j)) = vLastKeyValue1(j) And prsDataSet2.Fields(aszMergeKeyField(j)) <> vLastKeyValue2(j) Then
            Else
                bFind = False
                Exit For
            End If
        Next j
        If bFind Then
            prsDataSet2.MovePrevious        '上移一条，得出值
            bFind = True
        ...
        Next j
        If bFind Then
            prsDataSet1.MovePrevious        '上移一条，得出值
            bFind = True
            ...
            If bFind Then
                ...
                rsMerged.Update
                For j = 1 To nCount
                    vLastKeyValue1(j) = prsDataSet2.Fields(aszMergeKeyField(j))
                Next j
                prsDataSet1.MoveNext
                prsDataSet2.MoveNext
                GoTo NextLoop
        ...
        '注:为了修改简单起见,只按第一个字段进行排序
        If (pbDescSort And (prsDataSet1(aszMergeKeyField(1)) >= prsDataSet2(aszMergeKeyField(1)))) _
            Or (Not pbDescSort And (prsDataSet1(aszMergeKeyField(1)) <= prsDataSet2(aszMergeKeyField(1)))) Then
            '降序时，如果第一个值比第二个值大，取第一个记录集的值，第二个记录集置空
            '升序时，如果第一个值比第二个值小，取第一个记录集的值，第二个记录集置空
            rsMerged.AddNew             '赋记录1值，记录2为空
            For Each oField In prsDataSet1.Fields
                rsMerged.Fields(oField.Name) = oField.Value
            Next
            rsMerged.Update
            For j = 1 To nCount
                vLastKeyValue1(j) = prsDataSet1(aszMergeKeyField(j))
            Next j
            prsDataSet1.MoveNext
            GoTo NextLoop
        End If
        ...
        If (pbDescSort And (prsDataSet1(aszMergeKeyField(1)) <= prsDataSet2(aszMergeKeyField(1)))) _
            Or (Not pbDescSort And (prsDataSet1(aszMergeKeyField(1)) >= prsDataSet2(aszMergeKeyField(1)))) Then
            '降序时，如果第一个值比第二个值小，取第二个记录集的值，第一个记录集置空
            '升序时，如果第一个值比第二个值大，取第二个记录集的值，第一个记录集置空
...
EndReturn:
    Set MergeMultiRecord = rsMerged
    Set rsMerged = Nothing
    If MergeMultiRecord.RecordCount > 0 Then MergeMultiRecord.MoveFirst
End Function
...
Public Function GetStationNameInCbo(pszText As String) As String
    ...
    End If
End Function
Public Function CompareASCII(pnAscii As Integer) As Boolean
    CompareASCII = False
    If pnAscii >= 48 And pnAscii <= 57 Then
        CompareASCII = True
    ElseIf pnAscii >= 65 And pnAscii <= 90 Then
        CompareASCII = True
    ElseIf pnAscii >= 97 And pnAscii <= 122 Then
        CompareASCII = True
    End If
End Function
